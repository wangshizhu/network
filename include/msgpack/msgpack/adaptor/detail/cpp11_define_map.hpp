...
#ifndef MSGPACK_MAX_KEY_LEN
#define MSGPACK_MAX_KEY_LEN 128
#endif // !MSGPACK_MAX_KEY_LEN
...
struct define_map {
    ...
    void msgpack_unpack(msgpack::object const& o) const
    {
        ...
        for (uint32_t i = 0; i < o.via.map.size; ++i) {
			/* Bug Fix 2018.04.03
			*  Bug found by ChuRunMin, fixed by Wuding, rechecked by ChuRunMin.
			*  Bug Detail : non-string type key will make msgpack crash, we need some check.
			*/

			// we assume that the key is string type.
			if (o.via.map.ptr[i].key.type != msgpack::type::STR) continue;
			// we assume that the key is not too long, default length is 128, you can change this by define MSGPACK_MAX_KEY_LEN.
			if (o.via.map.ptr[i].key.via.str.size >= MSGPACK_MAX_KEY_LEN) continue;

// output message to make sure we have complied the fixed version, you can turn off this by define MSGPACK_NO_BUGFIX_MSG.
#ifndef MSGPACK_NO_BUGFIX_MSG
			#pragma message("msgpack non-string type key bug fixed.")
#endif // !MSGPACK_NO_BUGFIX_MSG
